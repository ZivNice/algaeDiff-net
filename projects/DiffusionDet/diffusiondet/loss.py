...
# projects/DiffusionDet/diffusiondet/loss.py
# @MODELS.register_module()
# class EnhancedDiffusionDetCriterion(DiffusionDetCriterion):
#     """Enhanced DiffusionDet Criterion with auxiliary loss."""
#
#     def __init__(self,
#                  num_classes,
#                  assigner,
#                  aux_weight=0.3,
#                  aux_loss=dict(type='MSELoss', reduction='mean'),
#                  deep_supervision=True,  # 添加这个参数
#                  **kwargs):
#         super().__init__(num_classes=num_classes, assigner=assigner, **kwargs)
#         self.aux_weight = aux_weight
#         self.aux_loss = MODELS.build(aux_loss)
#         self.deep_supervision = deep_supervision  # 保存参数
#
#     def forward(self, outputs, batch_gt_instances, batch_img_metas):
#         # 1. 计算原始 DiffusionDet 损失
#         losses = super().forward(outputs, batch_gt_instances, batch_img_metas)
#
#         # 2. 计算辅助损失(如果提供)
#         if 'aux_pred' in outputs and 'aux_gt' in outputs:
#             aux_loss = self.aux_loss(outputs['aux_pred'], outputs['aux_gt'])
#             losses['loss_aux'] = self.aux_weight * aux_loss
#
#             # 如果有深度监督,也计算辅助分支的损失
#             if self.deep_supervision and 'aux_outputs' in outputs:
#                 for i, aux_outputs in enumerate(outputs['aux_outputs']):
#                     if 'aux_pred' in aux_outputs and 'aux_gt' in aux_outputs:
#                         aux_loss = self.aux_loss(
#                             aux_outputs['aux_pred'],
#                             aux_outputs['aux_gt']
#                         )
#                         losses[f's.{i}.loss_aux'] = self.aux_weight * aux_loss
#
#         return losses
...
class EnhancedDiffusionDetCriterion(DiffusionDetCriterion):
    ...
    def __init__(self,
                 ...
                 deep_supervision=True,
                 aux_weight=0.3,
                 density_loss=dict(type='MSELoss', reduction='mean'),
                 use_adem=False,
                 **kwargs):
        super().__init__(
            ...
            **kwargs)
        self.aux_weight = aux_weight
        self.density_loss = MODELS.build(density_loss)
    ...
    def forward(self, outputs, batch_gt_instances, batch_img_metas):
        ...
        # 2. 如果启用ADEM，计算密度估计损失
        if self.use_adem and 'aux_pred' in outputs and 'aux_gt' in outputs:
            pred_density = outputs['aux_pred']
            gt_density = outputs['aux_gt']

            # 确保gt_density的形状与pred_density匹配
            if gt_density.shape != pred_density.shape and gt_density.dim() != pred_density.dim():
                # 如果gt_density是[B, H, W]而pred_density是[B, 1, H, W]
            ...
            # 计算密度估计损失
            density_loss = self.density_loss(pred_density, gt_density)
            losses['loss_density'] = self.aux_weight * density_loss

            # 如果有深度监督,也计算辅助分支的损失
            if self.deep_supervision and 'aux_outputs' in outputs:
                for i, aux_outputs in enumerate(outputs['aux_outputs']):
                    if 'aux_pred' in aux_outputs and 'aux_gt' in aux_outputs:
                        aux_pred = aux_outputs['aux_pred']
                        aux_gt = aux_outputs['aux_gt']

                        # 同样确保形状匹配
                        if aux_gt.shape != aux_pred.shape and aux_gt.dim() != aux_pred.dim():
                        ...
                        aux_density_loss = self.density_loss(aux_pred, aux_gt)
                        losses[f's.{i}.loss_density'] = self.aux_weight * aux_density_loss
